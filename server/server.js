...
io.on('connection', (socket) => {
  ...
  socket.on('register_user', (userId) => {
    if (userId) {
      ...
      console.log(
        `🟢 User ${userId} online with socket ${socket.id}`
      );
    }
  });
  ...
  socket.on('send_message', async (data) => {
    ...
    try {
      ...
      try {
        ...
        console.warn(
          'DB Save fallback in socket, returning in-memory message object'
        );
      }
  ...
  socket.on(
    ...
    async ({
      ...
      try {
        ...
        console.error(
          'Error in react_message handler:',
          err
        );
      }
    }
    ...
    async ({ from_user_id, to_user_id }) => {
      try {
        ...
        const senderSocketId =
          userSocketMap.get(from_user_id);
        ...
        console.error(
          'Error in mark_seen socket handler:',
          err
        );
      }
    }
  ...
  socket.on(
    ...
    ({
      ...
      if (recipientSocketId) {
        ...
        console.log(
          `📞 Call initiated from ${socket.userId} to ${to_user_id} (${call_type})`
        );
      } else {
        socket.emit(
          'call_user_offline',
  ...
  socket.on(
    ...
    ({ to_user_id, answer }) => {
      ...
      if (recipientSocketId) {
        ...
        console.log(
          `✅ Call answered by ${socket.userId} for ${to_user_id}`
        );
      }
    }
  );
...
connectDB()
  .then(() => {
    server.listen(PORT, () => {
      console.log(
        `🚀 Ryzo Server & Socket.io listening on port ${PORT}`
      );
    });
  })
  .catch((err) => {
    console.error(
      'Failed to connect to DB, starting server fallback...',
      err
    );

    server.listen(PORT, () => {
      console.log(
        `🚀 Server running on port ${PORT} (fallback mode)`
      );
    });
  });
